...
const AttendanceReport = () => {
  ...
  return (
    <main className='w-screen min-h-screen bg-dark'>
      <ViewHeader title="Attendance Report" />
      <section className='flex flex-col justify-center items-center gap-3'>
        <form className='w-11/12 mx-auto flex flex-col gap-4 p-4'>
          ...
          {
            allTime ? (
              <a download className='bg-pink text-white font-bold p-3 rounded-md text-center' href="/api/download?type=attendance&all=true">
                Download
              </a>
            ) : (
              <a download className='bg-pink text-white font-bold p-3 rounded-md text-center' href={`/api/download?type=attendance&start=${start}&end=${end}`}>
                Download
              </a>
            )
          }
